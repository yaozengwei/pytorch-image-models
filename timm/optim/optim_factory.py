...
from .scaledadam import ScaledAdam
...
def create_optimizer_v2(
    ...
    if isinstance(model_or_params, nn.Module):
        ...
        elif opt_lower == "scaledadam":
            assert lr is not None  # should have no effect
            parameters = get_parameter_groups_with_lrs(
                model_or_params, lr=lr, include_names=True,
            )
            weight_decay = 0.
    ...
    # bitsandbytes optimizers, require bitsandbytes to be installed
        ...
        opt_args.pop('weight_decay')
        optimizer = ScaledAdam(parameters, clipping_scale=2.0, **opt_args)
